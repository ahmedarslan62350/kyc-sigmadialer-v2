...
export const registrationSchema = z.object({
  ...
  role: z
    .string()
    .trim()
    .min(1, { message: "Business role is required" })
    .max(100, { message: "Role cannot exceed 100 characters" }),
  
  callingLocation: z.enum(["USA", "Canada"], {
    message: "Calling location must be USA or Canada",
  }),
  ...
  teamsId: z
    .string()
    .trim()
    .min(1, { message: "Teams ID is required (e.g. corporate identifier)" })
    .max(100, { message: "Teams ID cannot exceed 100 characters" }),
